...
async function exchangeRate() {
	var bitcoinfetch = await fetch(
		'https://min-api.cryptocompare.com/data/price?fsym=USD&tsyms=BTC&api_key=782824d04d7b391c833edfbab9264c4548cf44248ff2b3fbf382250bccea56f7'
...
async function changePrice(package) {
	...
	if (package === 'premiumButton') {
		var premiumPrice = bitcoinRate * 500;
		...
		var vipPrice = bitcoinRate * 1000;
		amount.textContent = vipPrice;
		usd.textContent = 1000;
...
const stripe = Stripe(
	'pk_test_51Gsx6JJyRCyDOw0DXnApg528fwHVjqfXGDLDGjAsUJGH4ELEZWc9hOW85e4PSOPQt0iDqfH8w1UVTwmmMlnFMV0s00by1xmXp4'
);
...
fetch('/setup')
	...
	.then(function(json) {
		...
		// Setup event handler to create a Checkout Session when button is clicked
		document.getElementById('pro-plan-btn').addEventListener('click', function(evt) {
		...
		// Setup event handler to create a Checkout Session when button is clicked
		document.getElementById('vip-plan-btn').addEventListener('click', function(evt) {
